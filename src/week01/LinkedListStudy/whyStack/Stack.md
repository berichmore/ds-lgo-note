...
🎯 8-1. 이 챕터의 목적  
- Stack을 실전 문제 해결에 적용하는 능력 얻기  
- 코딩테스트에서 자주 나오는 괄호 검사, undo기능, 수식 계산 문제를 직접 설계
- Stack의 문제해결 패턴을 내 것으로 만들기

✅ 8-2. 괄호 유효성 검사(Valid Parentheses)

🔍문제 설명
> 문자열에 포함된 괄호가 유효한지 확인하라.   
> 괄호는 항상 열고 닫는 순서가 맞아야 한다.  
> 예: "()","([])", "{[()]}" -> ✅
> "(", "(]", "{[(]}]" -> ❌  
>
