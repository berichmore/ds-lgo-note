...
public class day42 {
    public static void main(String[] args) throws Exception {
        ...
        for(int i = 1; i <= N; i++){
            ...
            arrayList.add(arrayList.size() - num, i);
        }
        StringBuilder sb= new StringBuilder();
        for(int student: arrayList){
...
for문 두 개를 돌려서
아니야 for문은 필요없어  한 번만 선회하는거니까

    그냥 배열인 int[]를 쓴다면 단순 배열에서는 데이터를 중간에 삽입하게 된다면
    한 칸씩 뒤로 밀려나도록 해야하니까 (for문으로 하나하나 구현해야겠지 )
    이걸 자동으로 하게 할 수 있는 ArrayList를 쓰는 것이 편리하겠다
    ArrayList는 중간에 add를 해도 자동으로 내부적으로 처리해주니까.
